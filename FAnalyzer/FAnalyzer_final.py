...
class Fanalyzer(object):
	...
	def runAlpine(self, tname):
		...
		while self.pq:
			...
			if len(items) == 0:
				...
				itmList = xrange(0, lastItem)  # possible extension items of the itemset
                                #print("itmList before--",set(itmList))
				for itm in items:
					key, val = idm[itm]
		...
		#print 'number of slices: ', nSlices
                #print hist_counter, box_counter
	...
	def genChartObjs(self, slice, tidlist, tname):
		...
		num_fields = tableData['num_fields']
                timeseries_fields = tableData['timeseries_fields']
		nrecords = tableData['nRecords']
		...
		for key in num_fields:
			...
			chart_object['data'] = map(lambda x,y:[x,y], ticks, freqs)
                        if n_bins == 1:
                            # made 0.999 instead of 1, because we are only displaying charts with score < 1 at the moment.
                            score = 0.999
                        else:
		...
		#boxplot
                ...
                #time series plots for each numerical attribute
                #print "Reached before timeseries"


                for xkey in timeseries_fields:
                   #print("xkey is: ",xkey)
                   for ykey in num_fields:
                       #print("ykey is: ",ykey)
                       #print("In num field of time series....")
                       #vals = tableData[xkey]['values']
                        #if not isinstance(vals, Sequence) or len(vals) == 0:
                        #    continue
                       #x_data = operator.itemgetter(*tidlist)(vals)
                        #if not isinstance(x_data, Sequence) or len(x_data) == 0:
