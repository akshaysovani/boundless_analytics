...
class Fanalyzer(object):
	...
	def genChartObjs(self, slice, tidlist, tname):
		...
		# 1-D percentile plot of numerical features
		for key in num_fields:
                        vals = tableData[key]['values']
                        if not isinstance(vals, Sequence) or len(vals) == 0:
                                continue
				...
				for idx in xrange(0, nCategories):
					...
					if len(tidlistforval) > 0:
						...
						if len(y_data) > 0:
							...
							# boxdata.append([minval, lowerquar, medianval, upperquar, ma{0:.2f}xval])
							# boxdata.append(["{0:.2f}".format(minval), "{0:.2f}".format(lowerquar), "".format(medianval), "{0:.2f}".format(upperquar), "{0:.2f}".format(maxval)])
