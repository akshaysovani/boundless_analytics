...
def validatecsv():
    if request.method == 'POST':
        ...
        csv_specialcase_op_path = '/tmp/temp_special_upload_for_validation_' + file.filename + '.csv'
        ...
        specialcase = True         #Every case is now a special case
        linenumber = 0
        try:
            #ipfile = open(csv_path,'rb')
            opfile = open(csv_specialcase_op_path,'w')
            #for ipline in ipfile.readlines():
                #doublequotecount = ipline.count('\"')
            ...
            if specialcase:
                ipfile = open(csv_path,'rb')
                #linenumber = 0
                for ipline in ipfile.readlines(): #condition to handle double quotes because of comma in data value
                    linenumber += 1
                    foundfirstdoublequote = False
                    commacountafterfirstdoublequote = 0
                    index = 0
                    for currentchar in ipline:
                        if currentchar == '"':
                            if not foundfirstdoublequote:
                                foundfirstdoublequote = True
                                commacountafterfirstdoublequote = 0
                            else:
                                foundfirstdoublequote = False
                                if commacountafterfirstdoublequote == 1:
                                    ipline = ipline[:commaindex] + "-" + ipline[commaindex+1:]
                                if commacountafterfirstdoublequote > 1:
                                    print("in here")
                                    #raise BAServiceError("stop")
                                    raise BAServiceError("Please check if a particular column data value on line {0} has multiple commas present in it. If yes, please replace all those commas with the hyphen character. If no, please remove the double quotes on that line and try uploading again!!".format(linenumber),400)
                        if foundfirstdoublequote and currentchar == ',':
                            commaindex = index
                            commacountafterfirstdoublequote += 1
                            #ipline = ipline[:index] + "-" + ipline[index+1:]
                        index += 1
                    #print("special: ",ipline)
                    opfile.write(ipline)
            ipfile.close()
            opfile.close()
        except:
            raise BAServiceError("Please check if a particular column value on line {0} has multiple commas present in it. If YES, then please replace all those commas with the hyphen (-) character. For example, Original value: \"Transformers: The Game (XBox 360, PS2, PS3, Wii & PC Versions)\",  Value after replacement:  \"Transformers: The Game (XBox 360- PS2- PS3- Wii & PC Versions)\".  Kindly check and do this for all the lines in the dataset. Else if answer is NO, please remove the double quotes on the line {1} and try uploading again!!".format(linenumber,linenumber),400)
            #raise BAServiceError("File contains comma in data values for some attributes. Please remove those commas..")
        ...
        try:
            line_num = 0
            if not specialcase:
                ipfile = open(csv_path,'rb') #normal scenario
            else:
                ipfile = open(csv_specialcase_op_path,'rb')
            ...
            for ipline in ipfile.readlines():
                ...
                try:
                    changedline = unicode(changedline, 'utf-8')
                except (TypeError, NameError):
                    pass
                changedline = unicodedata.normalize('NFD', changedline)
                changedline = changedline.encode('ascii', 'ignore')
                changedline = changedline.decode("utf-8")
                #print("changed line: ",changedline)
                line_arr = changedline.split(",")
                outputline = ""
                #if line_num == 990:
                    #print("line beofre: ",line_arr)
                for col in line_arr:
                    #foir eachcharacter in col:
                    #if eachcharacter in notallowedcharactersarray:
                    #if line_num == 990:
                        #print("col: ",col)
                    col = col.replace(";","")
                    col = col.replace("'","")
                    ...
                    col = col.replace("\\","")
                    #if line_num == 990:
                        #print("changed column ",col)
                    outputline += col
                    outputline += ","
                outputline = outputline[:-1]
                #print("output line: ",outputline)
                #if line_num == 5378:
                #    print("ipline: {0}, line number: {1}".format(changedline,line_num))
                #break
                opfile.write(outputline)
                #opfile.write(u'\r')
                #opfile.write(u'\n')
                opfile.write(str("\n"))
            ipfile.close()
            ...
            raise BAServiceError("cant open/write file",400)

        try:
                ifile = open(csv_op_path, "rb")
            #with io.open(csv_op_path, 'rb') as f:
                ...
                for row in reader:
                    ...
                    if i == 0:
                        ...
                        #        raise BAServiceError("There exists data in the data set without column name to it!! Please add column name for that data", 400)
                            ...
                            if len(head) == 0:
                                raise BAServiceError("Column number {0} doesn't have header name to it. Please add header name to continue..".format(number_of_attributes_counter), 400)
...
def upload():
    ...
    try:
        ...
        for col_name in schema:
            ...
            if (schema[col_name] == "num"):
                ...
                try:
                    ...
                    schema[col_name] = "cat"
                    df[col_name] = df[col_name].astype(str)
            elif schema[col_name] == "cat":
                schema[col_name] = "cat"
                df[col_name] = df[col_name].astype(str)
            else:
                schema[col_name] = "timeseries"
                #print(col_name)
                df[col_name] = df[col_name].astype(str)
        ...
        for col_name in header:
            ...
            #for time series
                json_file["tables"][table_name]["attrTypes"][col_name] = "timeseries"
        # For attributes
        json_file["tables"][table_name]["attributes"] = header
...
def dataset_new(dataset_name):
    ...
    try:
        ...
        file_name_global = dataset_name
        print("File name global assigned: ",file_name_global)
...
def oneSearch(page):
    ...
    print("file name at line 1..",file_name)
    if (file_name == 'None'):
        ...
        try:
            ...
            print("file name after charts added",file_name)
            ...
            print("Column query before problem",column_query)
    ...
    try:
        ...
        if (plot_data_type == 'boxplot'):
            ...
            for chart_arr in sort_array:
                ...
                value[1] = round(sum(inner_dict.values()), 2)
            sort_array = sorted(sort_array, key = lambda chart_arr: chart_arr[0][1], reverse=True)
        elif (plot_data_type == 'timeseries'):
            plot_zero_value_list = plot_zero_data
            max_value_zero = -9999999999

            max_date_zero = plot_zero_value_list[0][0]
            max_value_zero = plot_zero_value_list[0][1]

            #print('max date zero', max_date_zero)

            for v in plot_zero_value_list:
                if v[1] > max_value_zero:
                    max_value_zero = v[1]
                    max_date_zero = v[0]
                    #max_date_zero = unicodedata.normalize('NFKD', max_date_zero).encode('ascii', 'ignore')
                    print('max date zero before change', max_date_zero)
                    max_date_zero = datetime.fromtimestamp(float(max_date_zero)/1000.0)
                    max_date_zero = max_date_zero.strftime("%Y-%m-%dT%H:%M:%S")
                    max_date_zero = datetime.strptime(max_date_zero, '%Y-%m-%dT%H:%M:%S')
                    print('max date zero after change', max_date_zero)

            print('updated max date zero', max_date_zero)

            #max_date_zero = max_date_zero[:-10]
            #max_date_zero_date_format = datetime.strptime(max_date_zero, '%Y-%m-%dT%H:%M:%S')
            max_date_zero_date_format = max_date_zero
            max_date_array = []
            charts_per_score = sort_array
            for chart_obj in charts_per_score:
                max_value = plot_zero_value_list[0][1]
                max_date = plot_zero_value_list[0][0]
                value = chart_obj[0]
                compare_list = value[0]
                for v in compare_list:
                    if v[1] > max_value:
                        max_value = v[1]
                        max_date = v[0]
                        #max_date = unicodedata.normalize('NFKD', max_date).encode('ascii', 'ignore')
                        print('max date  before change', max_date)
                        max_date = datetime.fromtimestamp(float(max_date)/1000.0)
                        max_date = max_date.strftime("%Y-%m-%dT%H:%M:%S")
                        max_date = datetime.strptime(max_date, '%Y-%m-%dT%H:%M:%S')
                #max_date = max_date[:-10]
                #max_date_date_format = datetime.strptime(max_date, '%Y-%m-%dT%H:%M:%S')
                max_date_date_format = max_date
                value[1] = abs((max_date_date_format - max_date_zero_date_format).days)
                max_date_array.append(value[1])
        ...
        if (file_name == 'None'):
            ...
            if (attributes is not None):
                ...
                if (chart_type == 'timeseries'):
                    scores_query += " AND plottype = 'timeseries'"
                    scores_query += ";"
                mycursor.execute(scores_query)
                ...
                for i in range(len(scores_results)):
                    ...
                    if (chart_type == 'boxplot' or chart_type == 'percentile' ):
                        ...
                        for d in range(0, len(all_data)):
                            timeseries_data_list.append(all_data[d][1])
                        max_local = max(timeseries_data_list)
                        min_local = min(timeseries_data_list)
                        if max_local == 0 and min_local == 0:
                            continue
                        if (max_local > max_y_scale):
                            max_y_scale = max_local
                        if (min_local < min_y_scale):
                ...
                if (chart_type == 'bargraph'):
                    ...
                    print('In timeseries now')
                    plot_zero_value_list = plotzero_chart[0]
                    max_value_zero = -9999999999
                    print('plot_zero_value_list: ',plot_zero_value_list)

                    max_date_zero = plot_zero_value_list[0][0]
                    max_value_zero = plot_zero_value_list[0][1]

                    #print('max_date_zero',max_date_zero)
                    #print('max_value_zero',max_value_zero)
                    print(type(max_date_zero))
                    ...
                    plot_zero_value_max_date = max_date_zero
                    #print('max date in plot 0:',max_date_zero)
                    max_date_zero = max_date_zero[:-10]
                    print('max date in plot 0:',max_date_zero)
                    max_date_zero_date_format = datetime.strptime(max_date_zero, '%Y-%m-%dT%H:%M:%S')
                    print('max date in date format plot 0:',max_date_zero_date_format)
                    ...
                    for chart_obj in charts_per_score:
                        ...
                        for v in compare_list:
                            if v[1] > max_value:
                                max_value = v[1]
                                max_date = v[0]
                        #print('max date in plot 1:', max_date)
                        max_date = max_date[:-10]
                        max_date_date_format = datetime.strptime(max_date, '%Y-%m-%dT%H:%M:%S')
                        value[1] = abs((max_date_date_format - max_date_zero_date_format).days)
                        max_date_array.append(value[1])
                    print('all max dates', max_date_array)
                    charts_per_score = sorted(charts_per_score, key = lambda chart_obj: chart_obj[0][1], reverse=True)

                    #print("Time series plot, so no change as of now in order")
