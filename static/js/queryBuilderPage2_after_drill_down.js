...
function showOrHideDrillDownButton(){ //called when clicked on ON/OFF
    if ($('#checkbox_drill').is(":checked")){
        //if (endReachedFlag){
        //    alert("Cannot drill down further!!! No slice available.");  
        //}else{
        shiftPlotZero(false);
        document.getElementById("drilldownbutton").style.visibility='visible';    
    }else{
        
        document.getElementById("drilldownbutton").style.visibility='hidden';
    }
}

function buttonCall(id){
    if (id === 'plotzerobutton'){ //shift plot zero
        shiftPlotZero(true);
    }else{ // drill down
        if(endReachedFlag){
            alert("Cannot drill down further!!! No slice available.");
        }else if(!($('#supervised_checkbox').is(":checked")) && chart_index === charts_slice_global.length-1){ //supervised mode is OFF and chart is the last one in order 
            alert("Please select a plot with a slice to drill down on!!");
            $('#checkbox_drill').prop('checked', false);
        }else{
            shiftPlotZero(false);
        }
    }
...
function shiftPlotZero(isOrig){
    if (endReachedFlag){
        if (isOrig){
            alert("No use of making the current plot a new plot zero!! Slice cannot be narrowed down further!!");
        }
        //else{
            //alert("Cannot drill down further!!! No slice available.");
        //}
    ...
    if (($('#supervised_checkbox').is(":checked")) || (!($('#supervised_checkbox').is(":checked")) && chart_index !== charts_slice_global.length-1)){ //no last chart  if unsupervised
        ...
        if(isOrig){
            ...
            new_plot_zero_data = JSON.parse(JSON.stringify(all_record_keeper[0][0][0]));
            //new_plot_zero_data = JSON.parse(JSON.stringify());
        }
        ...
        $.ajax({
            ...
            //pushing the upcoming record to record keeper only when shift plot zero button is called
            if(isOrig){
                var record = [];
                var plotzerorecord = [];
                var plotonerecord = [];
